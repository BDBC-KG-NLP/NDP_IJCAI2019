...
def main(_):
  ...
  new_modesaveDataset = '_'.join(['artifical',str(args.artifical_noise_weight),args.datasets,args.model_type,str(args.learning_rate),
                             str(args.rnn_size),str(args.attention_hidden_size),
                             str(args.type_dim) ,str(args.margin),'sl2',str(args.l2_loss_w),
                             str(args.max_neg_type),str(args.max_pos_type),
                             str(args.is_add_fnode),
                             'r',str(0),str(args.filter_threshold)
                    ])
...
#          print(set(new_train_out_i))
#          print(set(train_out_re[i])) 
...
#          print('---------------------')
  ...
  if args.artifical_noise_weight != 0.0:
    ...
    cPickle.dump(params,open(args.dir_path + args.datasets+'/'+args.version_no+'/result/'+'decode_noise.p_'+new_modesaveDataset,'wb'))
    print(params)
  
  print('new increase_delete_num:',increase_delete_num)
  print('---------------------')
  assert(len(new_gen_train_out)==model_args.training_data_nums)
  cPickle.dump(new_gen_train_out,open(args.dir_path + args.datasets+'/'+args.version_no+'/iter_generated/'+'new_gen_train_out.p_'+new_modesaveDataset,'wb'))
  print('right_2_wrong_all:',right_2_wrong_all/model_args.training_data_nums)
  ...
  cPickle.dump(param_total,open(args.dir_path + args.datasets+'/'+args.version_no+'/result/'+'right2wrong.p_'+new_modesaveDataset,'wb'))
