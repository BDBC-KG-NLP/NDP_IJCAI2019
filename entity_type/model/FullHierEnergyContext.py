...
class FullHierEnergyContext(Model):
  def __init__(self,model_args):
    super(FullHierEnergyContext, self).__init__()
  ...
  def _init_main(self):
    ...
    if self.args.is_training == True:
      ...
      self.ent_ctx_t2 = tf.nn.l2_normalize(tf.layers.dense(self.ent_ctx_feature,self.args.type_dim,tf.nn.relu),-1) 
      self.ent_w_2 = tf.expand_dims(self.ent_ctx_t2,-1)  #(batch_size,type_dims,1)  
      
      #(batch,max_pos_type,1)
      self.a_norm = tf.nn.softmax(tf.einsum('aij,ajk->aik',self.pos_types_embed,self.ent_w_2)[:,:,0],-1)
