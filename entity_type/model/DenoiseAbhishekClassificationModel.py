...
import modified_hinge_loss


class DenoiseAbhishekModel(Model):
  def __init__(self,model_args):
    super(DenoiseAbhishekModel, self).__init__()
  ...
  def _init_placeholder(self):
    ...
    self.clean = tf.placeholder(tf.bool,[None],name='clean')  #we add this parameters
  ...
  def cl_loss_fn(self,embedded,return_intermediate=False):
    ...
    prediction_l1 =tf.contrib.layers.fully_connected(self.input_total,500,activation_fn=None,biases_initializer=None)
    ...
    loss = modified_hinge_loss.loss(logits,
                                    labels,
                                    self.clean,
                                    target_dim=self.args.class_size,
                                    use_clean=self.args.use_clean)
