...
class ComplExContextModel(Model):
  def __init__(self,model_args):
    super(ComplExContextModel, self).__init__()
  ...
  def cl_loss_fn(self,embedded,return_intermediate=False):
    ...
    if self.args.is_training == True:
      ...
      self.ment_feature_rel_2 = tf.expand_dims(tf.layers.dense(self.input_total,self.args.type_dim,
                                            kernel_initializer=tf.contrib.layers.xavier_initializer()),-1)
      self.ment_feature_img_2 = tf.expand_dims(tf.layers.dense(self.input_total,self.args.type_dim,
                                            kernel_initializer=tf.contrib.layers.xavier_initializer()),-1)
      
      #(batch,max_pos_type,1)
      a_weight =  tf.einsum('aij,ajk->aik',self.pos_types_rel_embed,self.ment_feature_rel_2) +\
                             tf.einsum('aij,ajk->aik',self.pos_types_img_embed,self.ment_feature_img_2)
      self.a_norm = tf.nn.softmax(a_weight[:,:,0],-1)
      print(self.a_norm)
